...
@NamedQueries({
    ...
    , @NamedQuery(name = "PersonDTO.findByNumber", query = "SELECT NEW dto.PersonDTO(p.id, p.firstName, p.lastName, p.email, p.address, p.hobbies, p.phones) FROM Person p WHERE :number MEMBER OF p.phones")
